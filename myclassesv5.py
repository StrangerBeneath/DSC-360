...
class Normalize_Corpus():
    ...
    def normalize(self, corpus, html_stripping=True, contraction_expansion=True,
        ...
        def normalize_corpus(corpus, html_stripping=False, contraction_expansion=False,
            ...
            from tqdm import tqdm
            import numpy as np
            pbar = tqdm(total=9, desc='Cleaning', colour='green')
            if html_stripping:
    ...
    def white_space_remover(self, df):
        ...
        tqdm_pandas = tqdm.pandas(desc="Striping Whitespace", colour='green') # Wrap dataframe rows with tqdm iterator
        df.progress_apply(lambda x: x.strip() if isinstance(x, str) else x)  # Strip whitespace
...
class Parse_Dialogue():
    ...
    def character_dialogue(self)-> pd.DataFrame:
        ...
        print('\n',movie_name)
        for character in tqdm(characters, desc='Parsing Character Dialogue', colour='green'):  # iterate over each character and extract their dialogue
            pattern = r'{}([\s\S]*?)(?=[A-Z]+\n)'.format(
                character)  # create a regex pattern to match the character with dialogue
            ...
            dfs.append(pd.DataFrame.from_dict(
                dialog_dict))  # create a DataFrame from the dictionary and append it to the list of DataFrames
